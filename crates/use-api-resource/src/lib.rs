...
text_newtype!(ApiResourceName);
text_newtype!(ApiResourceId);
text_newtype!(ApiResourceCollection);
text_newtype!(ApiResourcePath);
text_newtype!(ApiResourceRelationship);
...
impl fmt::Display for ApiResourceAction {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(self.as_str())
    }
}
...
pub struct PrimitiveMetadata {
    ...
    kind: ApiResourceAction,
}

impl PrimitiveMetadata {
    /// Creates primitive metadata.
    #[must_use]
    pub const fn new(name: ApiResourceName, kind: ApiResourceAction) -> Self {
        Self { name, kind }
    }

    /// Returns the primary text value.
    #[must_use]
    pub const fn name(&self) -> &ApiResourceName {
        &self.name
    }

    /// Returns the primary label.
    #[must_use]
    pub const fn kind(&self) -> ApiResourceAction {
        self.kind
    }
}
...
mod tests {
    ...
    fn parses_and_displays_text() -> Result<(), ApiPrimitiveError> {
        let value = ApiResourceName::new("users")?;

        assert_eq!(value.as_str(), "users");
        assert_eq!(value.to_string(), "users");
        assert_eq!("users".parse::<ApiResourceName>()?, value);
        Ok(())
    }

    #[test]
    fn rejects_empty_text() {
        assert_eq!(ApiResourceName::new(""), Err(ApiPrimitiveError::Empty));
    }

    #[test]
    fn parses_and_displays_labels() -> Result<(), ApiPrimitiveError> {
        let kind = "read".parse::<ApiResourceAction>()?;

        assert_eq!(kind, ApiResourceAction::Read);
    ...
    fn creates_metadata() -> Result<(), ApiPrimitiveError> {
        let metadata =
            PrimitiveMetadata::new(ApiResourceName::new("users")?, ApiResourceAction::default());

        assert_eq!(metadata.name().as_str(), "users");
        assert_eq!(metadata.kind(), ApiResourceAction::default());
        Ok(())
    }
}
